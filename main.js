// ── READ CARS FROM HTML ──
// All car data comes from #car-data <li> elements via data-* attributes.
// No car info is hardcoded here.
const carItems = Array.from(document.querySelectorAll('#car-data li'));
const cars = carItems.map(li => ({
...
function renderSpotlight(c) {
  ...
  document.getElementById('splSpecs').innerHTML   = `
    <div class="spl-spec"><div class="s-val">${c.fuel}</div><div class="s-key">Fuel Type</div></div>
    <div class="spl-spec"><div class="s-val">${c.transmission}</div><div class="s-key">Transmission</div></div>
    <div class="spl-spec"><div class="s-val">${c.seats}</div><div class="s-key">Capacity</div></div>
    <div class="spl-spec"><div class="s-val">${c.badge}</div><div class="s-key">Status</div></div>`;
...
document.getElementById('spotlight').addEventListener('touchend',   e => {
  ...
  if (Math.abs(diff) > 50) {
    if (diff > 0) document.getElementById('showNext').click();
    else          document.getElementById('showPrev').click();
  }
});

// ── CALCULATOR ──
document.getElementById('rTerm').oninput = () =>
  document.getElementById('termVal').textContent = document.getElementById('rTerm').value;
document.getElementById('rRate').oninput = () =>
  document.getElementById('rateVal').textContent = parseFloat(document.getElementById('rRate').value).toFixed(1);
...
function closeMob() {
  ...
  document.getElementById('hIcon').className = 'fas fa-bars';
}
...
function submitCF() {
  ...
  if (!n || !e || !m) {
    el.style.display = 'block'; el.style.color = '#e07070';
    el.textContent = '⚠ Please fill in all required fields.'; return;
  }
  el.style.display = 'block'; el.style.color = '#6be09e';
  el.textContent = '✅ Message sent! Alex will respond shortly.';
...
const cObs = new IntersectionObserver(entries => {
  ...
  document.querySelectorAll('[data-count]').forEach(el => {
    ...
    const timer = setInterval(() => {
      cur += step; if (cur >= t) { cur = t; clearInterval(timer); }
      el.textContent = Math.floor(cur) + (t > 100 ? '+' : '');
    }, 30);
  });
  cObs.disconnect();
}, { threshold: .5 });
const st = document.querySelector('.hero-stats');
if (st) cObs.observe(st);
...
// ── CHATBOT ──
const bR = {
  suv:       "Great pick! I have several SUVs — Toyota Fortuner, Honda CR-V, Ford Everest, Mitsubishi Montero Sport, and Lexus RX. Which brand interests you?",
  ...
  financing: "Use the calculator on this page to estimate your monthly payments, or contact Alex directly for bank pre-approval and personalized rates.",
  price:     "Prices start from ₱1.5M to ₱4.5M+. What's your budget range?",
  def:       "For the fastest response, Viber Alex at +63 912 345 6789 or fill out the contact form below. 😊"
};
function getBR(t) {
  t = t.toLowerCase();
...
function sendChat() {
  ...
  document.getElementById('cqBtns').innerHTML = '';
